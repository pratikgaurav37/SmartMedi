...
export async function sendTestMessage(chatId: string): Promise<{ success: boolean; error?: string }> {
  if (!bot) {
  ...
  try {
    ...
    if (error.response?.body?.description) {
      ...
      if (description.includes('chat not found')) {
        errorMessage = 'Chat not found. Please start a conversation with the bot by sending /start on Telegram first.';
      } else if (description.includes('bot was blocked')) {
        errorMessage = 'You have blocked this bot. Please unblock it on Telegram and try again.';
        ...
        errorMessage = 'Bot token is invalid. Please check your TELEGRAM_BOT_TOKEN configuration.';
      } else {
